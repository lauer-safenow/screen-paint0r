...
if (!gotLock) {
  ...
  app.whenReady().then(() => {
    try {
      ...
      log(`Registering shortcuts: ${SHORTCUTS.TOGGLE_DRAW}, ${SHORTCUTS.CLEAR_ALL}`);
      ...
      log(`Shortcut registration - draw: ${drawReg}, clear: ${clearReg}`);

      log('Creating tray...');
      createTray(() => toggleDrawMode(), () => clearAll());
